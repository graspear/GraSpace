...
usage(){
  ...
  echo
  echo "This program helps to manage the setup/teardown of the docker containers for running GraSpace. We recommend that you read the full documentation of docker at https://docs.docker.com if you want to customize your setup."
  echo
  echo "Command list:"
  echo "	start [options]		Start graspace"
  echo "	stop			Stop graspace"
  echo "	down			Stop and remove graspace's docker containers"
  echo "	update			Update graspace to the latest release"
  echo "	liveupdate		Update graspace to the latest release without stopping it"
  ...
  echo "	resetadminpassword \"<new password>\"	Reset the administrator's password to a new one. graspace must be running when executing this command and the password must be enclosed in double quotes."
  echo ""
  echo "Options:"
  echo "	--port	<port>	Set the port that graspace should bind to (default: $DEFAULT_PORT)"
  echo "	--hostname	<hostname>	Set the hostname that graspace will be accessible from (default: $DEFAULT_HOST)"
  ...
  echo "	--default-nodes	The amount of default NodeODM nodes attached to graspace on startup (default: $DEFAULT_NODES)"
  echo "	--with-micmac	Create a NodeMICMAC node attached to graspace on startup. Experimental! (default: disabled)"
  ...
  echo "	--dev	Enable development mode. In development mode you can make modifications to graspace source files and changes will be reflected live. (default: disabled)"
  ...
  echo "	--detached	Run graspace in detached mode. This means graspace will run in the background, without blocking the terminal (default: disabled)"
...
check_command(){
	...
	if [[ $not_found ]]; then
		...
		if [[ -n "$3" ]]; then
			...
			check_msg_result="\033[91m can't find $1! Check that the program is installed and that you have added the proper path to the program to your PATH environment variable before launching graspace. If you change your PATH environment variable, remember to close and reopen your terminal. $2\033[39m"
		fi
	fi
...
start(){
	...
	if [[ $dev_mode = true ]]; then
		echo "Starting graspace in development mode..."
		down
	else
		echo "Starting Graspace..."
	fi
	echo ""
	...
	if [ "$WO_SSL" = "YES" ]; then
		...
		if [ "$WO_HOST" = "localhost" ]; then
			echo -e "\033[91mSSL is enabled, but hostname cannot be set to $WO_HOST. Set the --hostname argument to the domain of your graspace server (for example: www.mywebodm.org).\033[39m"
			exit 1
		fi
...
rebuild(){
	...
	echo -e "\033[1mDone!\033[0m You can now start Graspace by running $0 start"
}
...
run_tests(){
    ...
    if [[ -f /.dockerenv ]]; then
        ...
        run "$docker_compose exec webapp /bin/bash -c \"/webodm/graspace.sh test\""
    fi
}
...
resetpassword(){
	...
	if [[ -n "$newpass" ]]; then
		...
		if [[ -z "$container_hash" ]]; then
			echo -e "\033[91mCannot find webapp docker container. Is graspace running?\033[39m"
			exit 1
		fi
...
if [[ $1 = "start" ]]; then
	...
	environment_check
	echo "Restarting Graspace..."
	down
	start
elif [[ $1 = "down" ]]; then
	environment_check
	echo "Tearing down Graspace..."
	down
elif [[ $1 = "rebuild" ]]; then
	environment_check
	echo  "Rebuilding Graspace..."
	rebuild
elif [[ $1 = "update" ]]; then
	environment_check
	down
